      ...
      <ScrollArea className="flex-1 px-4">
        <div className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-4 pb-6">
          {TEXT_PRESETS.map((preset, index) => (
            <PresetItem key={index} preset={preset} onAdd={handleAddText} />
          ))}
        </div>
      </ScrollArea>
    </div>
...
function PresetItem({ preset, onAdd }: { preset: (typeof TEXT_PRESETS)[number]; onAdd: (preset: (typeof TEXT_PRESETS)[number]) => void }) {
